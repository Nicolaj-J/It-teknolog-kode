...
def forside():   #Funktionen rendere forsiden og sender variabler med database med samt numerisk værdi af dags dato
    data = get_dbforside()
    print(data)
    return render_template("Forside.html", sort_data = data, var = timedelta.days)        #rendere forside.html samt definere sort_data variablen til at være det samme som data. Dette gør at sort_data variablen kan bruges i html koden. samme med daysdelta
def get_dbforside(): #Samler information fra databasen
    ...
    if db is None:                              #Kigger på om der er en database forbindelse
        ...
        cursor.execute("select * from productbatch")           #opretter forbindelse og Køre queryen
